...
export const forgetpassFindAccoutn = (email, toast) => dispatch => {
  ...
  return axios
    .post(`${api}/users/password/forgot`, email)
    .then(res => {
      console.log(res.data);
...
export const confirmpassFindAccoutn = (password, confirmPassword, otp, toast) => dispatch => {
  ...
  return axios
    .post(`${api}/users/password/reset/${otp}`, { password, confirmPassword })
    .then(res => {
      console.log(res.data);
...
export const immediatelyLoginAfterRegister = token => dispatch => {
  dispatch({ type: IMMEDIATELY_LOGIN_AFTER_RESGISTER, payload: token });
};
